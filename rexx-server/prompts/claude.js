module.exports = function getPrompt(review, type) {
	...
	Your task is to detect emotional risk and causality. Focus on unmet expectations, forced choices, and trust violations.
	DO NOT respond with summaries, empathy, or prose. 
	In addition, identify the customer’s current behavioural mode based on their tone and language. Choose **only one** of the following modes from 
		...
		Your entire output must be a valid JSON object matching this format:
	OUTPUT FORMAT:
	{
	...
	Your job is not to explain, soften, or reflect. Just return valid JSON.
	`.trim();
};